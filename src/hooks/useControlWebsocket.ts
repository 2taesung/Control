...
export const useControlWebSocket = ({ app, site, sector }: WsParams) => {
  ...
  const [webSocketScheduleData, setWebSocketScheduleData] = useState(null);
  // const [webSocketResValueData, setWebSocketResValueData] = useState(null);
    ...
    const setupWebSocket = (wsInstance: WebSocket) => {
      ...
      wsInstance.onmessage = event => {
        if (isMounted.current && isWebSocketOpen(wsInstance)) {
          const scheduleData = JSON.parse(event.data);
          const { type } = scheduleData;
          switch (type) {
            case 'resValuData':
              console.info('resValuData');
              break;
            case 'scheduleData':
              console.info('scheduleData');
              setWebSocketScheduleData(scheduleData);
              break;
            default:
